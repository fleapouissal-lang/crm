  ...
  const text = await generateSalesAgentChat({
    ...
    messages: [
      ...history.slice(-20),
      {
        role: "user",
        content: [
          "Nouveau message prospect (peut être plusieurs lignes — UN seul tour):",
          inboundText,
          "",
          glossProspectMessage(inboundText),
          "",
          `Langue à utiliser pour répondre: ${detected}`,
          "",
          "MISSION DE CE TOUR:",
          "1) Comprendre la question réelle du prospect.",
          "2) Y répondre clairement (action reply) — c’est la priorité.",
          "3) Ajouter au plus UNE question utile pour avancer.",
          ...
          freeSlots.length
            ? `Créneaux libres (ne pas inventer): ${freeSlots.join(" · ")}`
            : "",
          "",
          "Prix: JAMAIS montant/devis. Si prix → request_brief OU propose_meeting.",
          "",
          "Brief lead:",
          buildLeadBrief(ctx),
        ].join("\n"),
